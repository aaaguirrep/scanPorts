...
function help() {
    ...
    echo -e "\t${purpleColour}Example:${endColour}${grayColour} ./scanPorts.sh -t X.X.X.X ${yellowColour}o ${grayColour}./scanPorts.sh -t example.com${endColour}\n"
    exit 0
}

function checkPhase(){
	timeout 2 bash -c "echo > /dev/tcp/$target_host/$1" 2> /dev/null && echo $1 >> ports.tmp && echo -e "\n\t\t${redColour}[!] ${blueColour}Port $1 - ${greenColour}OPEN${endColour}" &
